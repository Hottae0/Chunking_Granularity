...
def graph_dir(output: Path, extraction_size: int, index_store: Path | None = None) -> Path:
    """Resolve a graph from the shared store, falling back to a run-local store."""
    return (index_store if index_store is not None else output / "indexes") / f"e{extraction_size}"
...
def build_graph(settings, documents, extraction_size: int, logger) -> Path:
    ...
    root = graph_dir(settings.output, extraction_size, settings.index_store)
