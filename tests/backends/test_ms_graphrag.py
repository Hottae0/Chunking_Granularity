...
from rq1.backends.ms_graphrag.indexer import REQUIRED_TABLES, _input_name, _patch_settings, graph_dir
from rq1.experiments.cache import guard_run
from scripts.utilities.prepare_subset import _validate_graph
from rq1.datasets.graphrag_bench import load_novel, relation_statements

ROOT=Path(__file__).resolve().parents[2]

class ServerTests(unittest.TestCase):
    def settings(self):
        return replace(load_settings(ROOT/'examples/synthetic/experiment.yaml'),backend='ms_graphrag',model='chat',
    ...
    def test_shared_index_store_resolution(self):
        settings = replace(self.settings(), index_store=ROOT/'runs'/'shared-test-graphs')
        self.assertEqual(
            graph_dir(settings.output, 256, settings.index_store),
            settings.index_store/'e256',
        )
