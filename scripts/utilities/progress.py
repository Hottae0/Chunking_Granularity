...
from rq1.backends.ms_graphrag.indexer import graph_dir
...
def snapshot(config: Path) -> str:
    ...
    for position, size in enumerate(settings.sizes, start=1):
        graph = _actual_graph(settings.output, size, settings.index_store)
    ...
    lines = [
        ...
        f"Index store: {(settings.index_store or settings.output / 'indexes').resolve()}",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/rq1_3x3.yaml"),
    )
