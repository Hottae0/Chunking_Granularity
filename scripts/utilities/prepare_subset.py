...
from rq1.backends.ms_graphrag.indexer import validate_graph
...
def prepare(config: str | Path, source: str | Path, include_partial: bool = True) -> dict:
    ...
    target = (settings.index_store or (settings.output / "indexes")).resolve()
    if not source.exists():
        raise FileNotFoundError(f"Source graph store does not exist: {source}")
    if (source / "indexes").is_dir():
        source = (source / "indexes").resolve()
